...
def main(args):
    ...
    # number of classes default to 2
    ...
    # target is composed of 1 and 0 s
    # make it 1 and -1 
    target = 2 * target - 1

    # TODO: Append a constant feature with value 1 to the end of every input data
    num_of_instances, num_of_features = data.shape
    bias = np.ones((num_of_instances, 1))
    ...
    while not done:
        # run over data in random order
        permutation = generator.permutation(num_of_instances)
        ...
        for i in permutation:
            lin_reg = weights.T @ data[i]
            if (lin_reg * target[i]) <= 0 :
