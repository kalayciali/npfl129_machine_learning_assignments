...
def main(args):
    ...
    for i in range(args.bootstrap_samples):
        indices = generator.choice(len(test_data), size=len(test_data), replace=True)
        test_X, test_y = test_data[indices], test_target[indices]

        for j in range(scores.shape[0]):
            # j is row
            scores[j, i] = sklearn.metrics.accuracy_score(test_y, models[j].predict(test_X))
    ...
    confidence_intervals = [
        (percent2_5[0], percent97_5[0]), # confidence interval of the first model
        (percent2_5[1], percent97_5[1]), # confidence interval of the second model
    ]
